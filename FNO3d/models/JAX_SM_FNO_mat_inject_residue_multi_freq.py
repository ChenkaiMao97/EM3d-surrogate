...
@eqx.filter_jit
def H_to_bc(Hz_R, Hz_I, dL, wl, yeex, yeey, EPSILON_0 = EPSILON_0):
    Hz = Hz_R + 1j*Hz_I
    # x = 1 / 2 * (eps_grid[:, :, 1:, :] + eps_grid[:, :, 0:-1, :]) # Material averaging
    top_bc = (Hz[0,1:-1]-Hz[1,1:-1])+1j*2*jnp.pi*jnp.sqrt(yeex[1,:])/wl*dL*1/2*(Hz[0,1:-1]+Hz[1,1:-1])
    bottom_bc = (Hz[-1,1:-1]-Hz[-2,1:-1])+1j*2*jnp.pi*jnp.sqrt(yeex[-1,:])/wl*dL*1/2*(Hz[-1,1:-1]+Hz[-2,1:-1])
    left_bc = (Hz[1:-1,0]-Hz[1:-1,1])+1j*2*jnp.pi*jnp.sqrt(yeey[:,1])/wl*dL*1/2*(Hz[1:-1,0]+Hz[1:-1,1])
    right_bc = (Hz[1:-1,-1]-Hz[1:-1,-2])+1j*2*jnp.pi*jnp.sqrt(yeey[:,-1])/wl*dL*1/2*(Hz[1:-1,-1]+Hz[1:-1,-2])
...
class FNO_multimodal_2d(eqx.Module):
    ...
    def __init__(self, args, *, key):
        keys = jax.random.split(key, num=10+2*args.num_fourier_layers)
        ...
        self.residue_weights = jax.random.uniform(keys[9+2*self.num_fourier_layers], (self.num_fourier_layers-1,))/1000

    @eqx.filter_jit
    def __call__(self, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, wl):
        ...
        for i in range(self.num_fourier_layers-1):
            ...
            x_eff = x.copy()
            if self.padding > 0:
                x_eff = x_eff[..., :-self.padding, :-self.padding]

            x_eff = jax.vmap(jax.vmap(self.fc1, 1, 1),2,2)(x_eff)
            x_eff = jax.nn.leaky_relu(x_eff, negative_slope=self.ALPHA)
            x_eff = jax.vmap(jax.vmap(self.fc2, 1, 1),2,2)(x_eff)
            x_eff = x_eff.transpose((1, 2, 0))

            residue = jnp.zeros(x_eff.shape)
            FD_Hy = H_to_H(x_eff[:, :, 0], x_eff[:, :, 1], dL, omega, yeex[:,1:-1], yeey[1:-1,:])
            FD_bc = H_to_bc(x_eff[ :, :, 0], x_eff[ :, :, 1], dL, wl, yeex[:,1:-1], yeey[1:-1,:])

            inner_residue = FD_Hy - x_eff[1:-1,1:-1,:]
            #inner_residue = 1/2*(yeex[:,None,1:-1,1:-1]+yeey[:,None,1:-1,1:-1])*(FD_Hy - x_eff.permute(0,3,1,2)[:,:,1:-1,1:-1])
            bc_residue = 500*(FD_bc - jnp.concatenate((top_bc[0,1:-1], bottom_bc[0,1:-1], left_bc[1:-1,0], right_bc[1:-1,0]), axis=-1))
            #bc_residue = FD_bc - jnp.concatenate((top_bc[:,0,1:-1], bottom_bc[:,0,1:-1], left_bc[:,1:-1,0], right_bc[:,1:-1,0]), axis=-1)

            residue = residue.at[1:-1,1:-1].set(inner_residue)
            residue = residue.at[0,1:-1].set(bc_residue[:,0:2])
            residue = residue.at[-1,1:-1].set(bc_residue[:,2:4])
            residue = residue.at[1:-1,0].set(bc_residue[:,4:6])
            residue = residue.at[1:-1,-1].set(bc_residue[:,6:8])
            residue = residue.at[0,0].set(1/2*(residue[0,1]+residue[1,0]))
            residue = residue.at[0,-1].set(1/2*(residue[0,-2]+residue[1,-1]))
            residue = residue.at[-1,0].set(1/2*(residue[-1,1]+residue[-2,0]))
            residue = residue.at[-1,-1].set(1/2*(residue[-1,-2]+residue[-2,-1]))
            ########################

            x = jax.nn.leaky_relu(x, negative_slope=self.ALPHA)

            processed_residue = jnp.clip(residue, a_min=0.8*jnp.min(residue), a_max=0.8*jnp.max(residue))
            processed_residue = processed_residue/jnp.mean(jnp.abs(processed_residue))
            processed_residue = jnp.pad(processed_residue.transpose((2, 0, 1)),  ((0,0), (0,self.padding), (0,self.padding)), mode='constant', constant_values = 0)

            x = x.at[-2:].set(self.residue_weights[i]*processed_residue)
    ...
    @eqx.filter_jit
    def eval_forward(self, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, wl):
        # Sx_f: [subdomain_size, subdomain_size,2]
        # Sy_f: [subdomain_size, subdomain_size,2]
        # source: [subdomain_size, subdomain_size, 2]
        # yeex: [subdomain_size, subdomain_size-2]
        ...
        return x, preprocessed, processed_residue
