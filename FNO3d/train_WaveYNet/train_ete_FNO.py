...
def main(args):
    ...
    model_path = args.model_saving_path + '/' + args.model_name + \
                                              "_domain_size_" + str(args.domain_sizex) + "_"+ str(args.domain_sizey) + "_"+ str(args.domain_sizez) + \
    ...
    for step in range(start_epoch, args.epoch):
        ...
        for idx, sample_batched in tqdm(enumerate(train_loader)):
            ...
            y_batch_train, yeex_batch_train, yeey_batch_train, yeez_batch_train = sample_batched['field'].cuda(), sample_batched['yeex'].cuda(), sample_batched['yeey'].cuda(), sample_batched['yeez'].cuda()
        ...
        for idx, sample_batched in enumerate(test_loader):
            y_batch_test, yeex_batch_test, yeey_batch_test, yeez_batch_test = sample_batched['field'].cuda(), sample_batched['yeex'].cuda(), sample_batched['yeey'].cuda(), sample_batched['yeez'].cuda()
            ...
            with torch.no_grad():
                ...
                if idx == 0:
                    ...
                    plot_3slices(yeex_batch_test[0,:,:,:].detach().cpu().numpy(), model_path+"/plots/epoch_"+str(step)+"_yeex.png")
