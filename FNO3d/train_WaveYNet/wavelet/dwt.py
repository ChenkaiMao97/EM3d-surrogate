...
import time
import pywt
from dwt_utils import prep_filt_sfb3d, prep_filt_afb3d
...
class DWTInverse3d_Laplacian(nn.Module):
    ...
    def forward(self, coeffs):
        ...
        for h in yh[::-1]:
            ...
            ll_diff = SFB3D_Laplacian.apply(ll, self.g0_dep, self.g0_col, self.g0_row, mode)
...
class DWTInverse3d(nn.Module):
    ...
    def forward(self, coeffs):
        ...
        for h in yh[::-1]:
            if h is None:
                h = torch.zeros(ll.shape[0], ll.shape[1], 7, ll.shape[-3], ll.shape[-2], ll.shape[-1], device=ll.device)
            ...
            if ll.shape[-1] > h.shape[-1]:
                ll = ll[..., :-1]
            ll = SFB3D.apply(ll, h, self.g0_dep, self.g1_dep, self.g0_col, self.g1_col, self.g0_row, self.g1_row, mode)
        return ll
...
class DWTForward3d_Laplacian(nn.Module):
    ...
    def forward(self, x):
        ...
        for j in range(self.J):
            ...
            ll_new = AFB3D_Laplacian.apply(ll, self.h0_dep, self.h0_col, self.h0_row, mode)
...
class DWTForward3d(nn.Module):
    ...
    def forward(self, x):
        ...
        for j in range(self.J):
            # Do 1 level of the transform
            ll, high = AFB3D.apply(ll, self.h0_dep, self.h1_dep, self.h0_col, self.h1_col, self.h0_row, self.h1_row, mode)
            yh.append(high)
...
def sfb1d(lo, hi, g0, g1, mode="zero", dim=-1):
    ...
    y = F.conv_transpose3d(lo, g0, stride=s, padding=pad, groups=C) + F.conv_transpose3d(hi, g1, stride=s, padding=pad, groups=C)
