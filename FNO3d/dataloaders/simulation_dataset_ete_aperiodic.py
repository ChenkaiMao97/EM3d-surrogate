...
class SimulationDataset(Dataset):
    def __init__(self, data_folder, z_min=22, z_max=54, xpml=10, ypml=10, total_sample_number = None, transform = None, threshold=1e-4):
        ...
        dataset_means = {
            ...
            '/media/lts0/chenkaim/3d_data/SR_aperiodic_TiO2': np.array([2.7787502e-04, 2.6493979e-04, 4.8106882e-05, 4.8290374e-05, 7.1477669e-05, 7.2270515e-05]),
            '/media/lts0/chenkaim/3d_data/SR_aperiodic_TiO2_no_src_in_PML': np.array([3.1097332e-04, 2.8942098e-04, 4.5864312e-05, 4.6515746e-05, 8.2552593e-05, 8.6597960e-05]),
            '/media/lts0/chenkaim/3d_data/SR_aperiodic_TiO2_no_touching_PML': np.array([3.1574394e-04, 2.8498229e-04, 4.7228663e-05, 4.8066977e-05, 8.7598106e-05, 9.1788046e-05]),
        }
    ...
    def get_means(self):
        ...
        for d in tqdm(self.directories):
            try:
                Ex = np.load(self.root_dir + '/' +d+'/'+'0.0001Ex.npy', mmap_mode='r').astype(np.complex64, copy=False)[self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max]
                Ey = np.load(self.root_dir + '/' +d+'/'+'0.0001Ey.npy', mmap_mode='r').astype(np.complex64, copy=False)[self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max]
                Ez = np.load(self.root_dir + '/' +d+'/'+'0.0001Ez.npy', mmap_mode='r').astype(np.complex64, copy=False)[self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max]
    ...
    def __getitem__(self, idx):
        ...
        yeex, yeey, yeez = yee[0,self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max], yee[1,self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max], yee[2,self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max]
        # print(f"yee shapes: yeex: {yeex.shape}, yeey: {yeey.shape}, yeez: {yeez.shape}")

        Ex = torch.tensor(np.load(self.root_dir + '/' +folder+'/'+'0.0001Ex.npy', mmap_mode='r')[self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max], dtype=torch.complex64)
        Ey = torch.tensor(np.load(self.root_dir + '/' +folder+'/'+'0.0001Ey.npy', mmap_mode='r')[self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max], dtype=torch.complex64)
        Ez = torch.tensor(np.load(self.root_dir + '/' +folder+'/'+'0.0001Ez.npy', mmap_mode='r')[self.xpml:-self.xpml,self.ypml:-self.ypml,self.z_min:self.z_max], dtype=torch.complex64)
